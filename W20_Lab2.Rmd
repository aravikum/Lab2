...
nrow(exl_df) #number of rows
ncol(exl_df) #number of columns
dim(exl_df) #The dimensions of the dataframe
colnames(exl_df) #Names of the columns
rownames(exl_df) #Names of the rows
glimpse(exl_df) #show all the columns of the dataframe
str(exl_df) #prints the structure of the dataframe
summary(exl_df) # displays 5 number summary and the mean for every column
```
...
```{r}
new_exl_df<- mutate(exl_df, #dataframe to use
       sample_kg = sample_weight_g / 1000, #first column modified with mutate
       acetate_mmol_kg = (acetate_mM*0.002)/sample_kg,
       butyrate_mmol_kg = butyrate_mM*0.002/sample_kg) #second column modified with mutate
...
new_exl_df %>%
  ...
  summarize(mean_butyrate = mean(butyrate_mM))
```
